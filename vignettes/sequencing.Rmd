...
`readDNAStringSet()` from the `r Biocpkg("Biostrings")` package.
...
Alternatively, we can use tools from the `r Biocpkg("Rsamtools")` package
to selectively load a subset of the sequences only.
...
The index is returned as a _GRanges_ object. Then we use `getSeq()` to
